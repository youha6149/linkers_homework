...
- **詳細**
  - CSVの読み込みを同期的に行っています。非同期処理との比較検討の結果、実行速度に大きな差がなかったため、保守性とコードの簡潔さを考慮して同期的な処理を採用しています。
...
- プラン1、2はCSV入力用インターフェースが必要であり、工数が多くなります。
- プラン2はCSV入力を前提としていますが、ユーザー操作を少なくするためのプランとして検討しました。

以上の検討結果から、最も工数が少ないプラン3をベースにすることを決定しました。また、転置インデックスファイルを事前に作成する必要がない場合でも、作成と検索をそれぞれ単一で実行できるように開発を進めました。

### 4日目

#### **CSV取得処理と実行ファイルの作成**

以下の疑問点に対していただいた回答を参考に、開発を進めました:
